...
**Files:**
- Create: `tests/android/responsive-layout.test.js`
...
describe('响应式布局 Android 断点', () => {
  it('should_detect_small_phone', () => {
    const width = 360;
    const breakpoint = width < 360 ? 'xs' : width < 480 ? 'sm' : width < 768 ? 'md' : width < 1024 ? 'lg' : 'xl';
...
Run: `npm test -- tests/android/responsive-layout.test.js`
Expected: PASS (12 tests)

- [ ] **Step 3: Commit**

```bash
git add tests/android/responsive-layout.test.js
git commit -m "test: add Android responsive layout tests"
```
