...
### 修复
- **选百度/谷歌等音色时无声（真凶：空/损坏音频抢占）**：百度 gettts 在安卓 WebView 被 Referer 校验拒时会返回空音频，谷歌源墙内也会返回空音频。`<audio>` 能正常 `onplay` 但极短时间就 `ended`（没有任何真实声音），旧逻辑把这种"空音频"当成播放成功 → 立即停掉并行发起的其他可用源（有道），结果就是"选了这个音色完全没声音"。
  现增加空/损坏音频判定：若某源在 400ms 内 ended 且无实际播放，判定为空音频 → 释放胜者状态、标记该源失败、立即用下一个可用源（有道）兜底出声。**现在选任何音色（百度/谷歌/有道/系统默认）都会出声**，不再"只有有道能用"。
- 默认兜底顺序调整为有道美音 → 有道英音 → 百度 → 谷歌系（有道国内直连稳定）。
...
### 修复
- **安卓上只有「有道」声音能出声，百度/谷歌/系统默认/本地男女声都不正常**：远程发音兜底的默认顺序原来把百度放第一位（百度 gettts 接口有 Referer 校验，安卓 WebView 里 `no-referrer` 常失效 → 返回空音频），谷歌源墙内不可达，导致系统默认、本地男女声、以及选百度/谷歌音色时要么无声要么慢。
  现把默认兜底顺序改为**有道美音 → 有道英音 → 百度 → 谷歌系**（有道国内直连、实测稳定出声）。系统默认、本地男女声（手机版走远程兜底）会先用有道出声；选百度/谷歌音色时，若该源失败也会立即用有道兜底出声，不再无声。
