...
### 修复
- **安卓「系统默认」「本地男女声」仍可能无声且无提示（v1.0.1 残留）**：
  1. **远程发音源全失败时死循环空等**：当本会话内所有远程发音源（百度/有道/谷歌）都被健康表标记失败后，旧的 `pickSrcs` 会回退到已失败的 `order[0]` 源继续空等，每次 3.2s 超时，单词要等 7 轮（约 22 秒）才走完降级链，表现成"点击后完全没声音也没提示"（Toast 要 20 多秒后才弹，用户早以为坏了）。现改为：候选源全失败时立即判该块失败、跳过降级，快速弹提示，不再空等。
  2. **远程 `<audio>` 首次播放被自动播放策略拦截（NotAllowedError）**：Android WebView 首次远程播放常被 autoplay 策略拦一次，`play()` 的 promise 被拒即静默失败。现自动重试最多 2 次（间隔 250ms，同时解锁 AudioContext），多数情况下第二次（用户手势内）即出声。
  3. **本地男声/女声（David/Zira）在手机版标注不可用**：这两个选项依赖 Windows 本地 server 的 SAPI 合成，手机上选了不会响。现手机版下拉框标注"（手机版不可用，需电脑 + server 运行）"，避免误选。
- 系统默认、本地男女声、百度/有道等在线音色在安卓版统一走「远程发音兜底」链路，行为一致：国内直连源（百度/有道）正常出声，谷歌系源（需翻墙）作为兜底。
